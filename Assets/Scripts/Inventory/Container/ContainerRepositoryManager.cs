...
{
    public class ContainerRepositoryManager : IRepositoryManager
    {
        ...
        public void SetContainer(ContainerItemRepository repository)
        {
            _repository = repository;
            Rebuild();
            Resize(repository.Config.Widht, repository.Config.Height);
        }

        public void SpawnRandomLoot(ContainerConfig config)
        {
            var tries = (config.Widht * config.Height) / 3;
            for (var i = 0; i < tries; i++)
            {
                TryAdd(config.AllowedItems[Random.Range(0, config.AllowedItems.Count)].CreateInstance());
            }
            _repository.isLootSpawned = true;
        }
