...
{
    public class PowerCellSlotRepositoryManager : IRepositoryManager
    {
         private Vector2Int _size = Vector2Int.one;
        private IPowerCellSlotRepository _repository;
        private Rect _fullRect;

        public PowerCellSlotRepositoryManager(IPowerCellSlotRepository repository)
        {
        ...
        public IPowerCellSlotRepository Repository => _repository;
        ...
        /// <inheritdoc />
        public bool CanRemove(IInventoryItem item) => Contains(item) && _repository.CanRemoveCellItem();

        /// <inheritdoc />
        public bool TryRemove(IInventoryItem item)
        {
            if (!CanRemove(item)) return false;
            if (!_repository.RemoveCellItem()) return false;
            Rebuild(true);
            onItemRemoved?.Invoke(item);
            PlayerPowerEventHolder.SendOnPowerCellRemoved(_repository.SlotId);
            return true;
        }
        ...
        {
            if (!CanDrop(item) || !_repository.DropCellItem())
            {
        ...
        {
            return _repository.CanAddCellItem(item) && _repository.IsEmpty;
        ...
        {
            if (!CanAddAt(item, point) || !_repository.AddCellItem(item))
            {
            ...
            PlayerPowerEventHolder.SendOnPowerCellAdded(_repository.SlotId);
            return true;
        }
        ...
        {
            return DoesItemFit(item) && _repository.CanAddCellItem(item);
        }
        ...
        public bool CanDrop(IInventoryItem item) =>
            Contains(item) && _repository.CanDropCellItem() && item.canDrop;
