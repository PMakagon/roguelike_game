...
{
    public class EquipmentSlotManager : IInventoryManager
    {
       private Vector2Int _size = Vector2Int.one;
        private IEquipmentSlotProvider _provider;
        private Rect _fullRect;

        public EquipmentSlotManager(IEquipmentSlotProvider provider, int width, int height)
        {
            _provider = provider;
            Rebuild();
            Resize(width, height);
        ...
        {
            allItems = new IInventoryItem[1]{_provider.GetEquipmentItem()};
        ...
        public IInventoryItem GetAtPoint(Vector2Int point)
        {
            ...
            {
                return _provider.GetEquipmentItem();
            }
            return null;
        }
        ...
        public bool TryRemove(IInventoryItem item)
        {
            ...
            if (!_provider.RemoveEquipmentItem()) return false;
            Rebuild(true);
        ...
        {
            if (!CanDrop(item) || !_provider.DropEquipmentItem()) 
			{
        ...
        {
            if (!_provider.CanAddEquipmentItem(item) || !_provider.IsEmpty)
            {
                return false;
            }
        ...
        {
            if (!CanAddAt(item, point) || !_provider.AddEquipmentItem(item)) 
			{
        ...
        public bool CanSwap(IInventoryItem item)
        {
            return _provider.InventoryRenderMode == InventoryRenderMode.Single &&
                DoesItemFit(item) &&
                _provider.CanAddEquipmentItem(item);
        }
        ...
        /// <inheritdoc />
        public bool CanRemove(IInventoryItem item) => Contains(item) && _provider.CanRemoveEquipmentItem();

        /// <inheritdoc />
        public bool CanDrop(IInventoryItem item) => Contains(item) && _provider.CanDropEquipmentItem() && item.canDrop;
